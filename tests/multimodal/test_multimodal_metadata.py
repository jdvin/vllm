...
ModelRegistry.register_model(
    "TestModel", TestModel, is_multimodal=True
)
...
def test_multimodal_metadata():
    ...
    engine.add_request("0", 
                       {"prompt_token_ids": [], "multi_modal_data": {"text_conditioning": "Hello there."}}, 
                       sampling_params)
    engine.step()
    step2_out = engine.step()
    assert step2_out[0].outputs[0].token_ids[0] == step2_out[0].outputs[0].token_ids[1] == 10
